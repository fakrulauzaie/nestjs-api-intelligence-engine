...
| Redlock critical sections         | `resource.redlock.using.v1`                                                                                                                                                                     | Package-proven `redlock` `using()` calls with bounded direct/immutable-const resource arrays and an inline arrow or function callback                                                                                       | Dynamic or oversized targets are diagnosed; callback references, lookalikes, unproven wrappers, acquisition, exclusivity, timing, contention, execution, renewal, and release are not inferred | `test/unit/extractors/redlock-critical-sections.test.ts`                                                                                                                                                  |
| Verified Redlock wrapper flows    | `resource.redlock.verified-wrapper.v1`                                                                                                                                                          | Exact checker-resolved repository methods; directly invoked callable parameter; up to three unchanged positional forwarding hops to package-proven `using()`; exact call site with inline callback                          | Names/configuration are not proof; transformed, stored, scheduled, spread, method-reference, ambiguous, external, cyclic, or bounded-out flows never authorize traversal                       | `test/unit/extractors/critical-section-wrapper-flow.test.ts`, `test/unit/extractors/critical-section-wrapper-propagation.test.ts`, and `test/unit/extractors/critical-section-wrapper-projection.test.ts` |
...
## Analysis v8 capability boundary
...
Analysis schema `8.0.0` preserves the analysis-v7 `outbound_http`, `in_process_event`,
...
Analysis v8 preserves package-proven non-relational resource inventory:
...
Analysis v7 introduced package-proven Redlock `using()` calls with up to
...
callback references, and lookalikes remain fail-closed gaps.

Analysis v8 additionally proves selected repository wrapper chains without wrapper
configuration or naming heuristics. A directly invoked callable parameter may
propagate backwards through at most three exact methods only when it is forwarded
unchanged at the same positional argument. At an exact caller, only an inline arrow
or function argument is opened; every other nested function remains closed. Wrapper
resources use a dynamic target and retain the complete caller-to-terminal evidence
chain. Terminal-connected ambiguity, unsupported callback shapes, cycles, and safety
limits are explicit diagnostics. See [Redlock Critical Sections](redlock-critical-sections.md)
and the [Phase W6 real-service validation](benchmarks/phase-w6-critical-section-wrapper-validation.md).
